...
function referchDtree(){
		...
		success:function(obj){
			...
			$.each(obj,function(){
				...
				if(this.id.length!=12){
					d.add(this.id,this.pid,this.name,this.id.length==8 ? "${ctx}/identity/popedom/getLoadPopedom.jspx?code="+this.id+"&id=${id}"+"&name="+this.name :"",this.name,"rightFrame");
				}
    ...
    <body class="easyui-layout" style="width:100%;height:100%;">
			...
			<div data-options="region:'center'" title="模块菜单"  >
			     <!-- 展示当前模块下的子模块  -->
			     <iframe scrolling="auto"  frameborder="0" name="rightFrame" id="sonModules"  width="100%" height="100%" ></iframe>
			</div>
	</body>
</html>
